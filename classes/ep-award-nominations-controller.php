	...
	if (!class_exists(epAwardNominationsDatabaseController)) {
		class epAwardNominationsDatabaseController {
			public function install_database() {
				...
				$wpdb->query( "CREATE TABLE IF NOT EXISTS {$wpdb->prefix}epan_nominations ( id INT(10) UNSIGNED NOT NULL AUTO_INCREMENT, awardID INT(10) UNSIGNED NOT NULL, categoryID INT(10) UNSIGNED NOT NULL, nominee VARCHAR(60), reason TEXT, nominatorFirst VARCHAR(60), nominatorLast VARCHAR(60), nominatorPhone VARCHAR(60), nominatorEmail VARCHAR(60), PRIMARY KEY (id) ) {$wpdb->get_charset_collate()}");
			...
			public function insert_test_data() {
				...
				$wpdb->query("INSERT INTO {$wpdb->prefix}epan_categories (id, awardID, title, description) VALUES (NULL, 1, 'Employee', 'Over 21 years of age')");
				$wpdb->query("INSERT INTO {$wpdb->prefix}epan_categories (id, awardID, title, description) VALUES (NULL, 1, 'Junior Employee', '21 years of age or younger')");
				...
				$wpdb->query("INSERT INTO {$wpdb->prefix}epan_questions (id, awardID, title, description) VALUES (NULL, 1, 'Employment History', 'Please tell us, briefly, the story of how you came to be employed by your business, what positions/roles you have held, and why you are the 2017 Goondiwindi Employee of the Year.')");
				$wpdb->query("INSERT INTO {$wpdb->prefix}epan_questions (id, awardID, title, description) VALUES (NULL, 1, 'Innovation History', 'Please tell us, briefly, the story of how your unique product/service came to be, and share some milestones or trials and tribulations.')");
				$wpdb->flush();
			}
